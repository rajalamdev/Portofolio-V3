...
// query
// http://localhost:1337/api/projects?populate[project_categories][populate]=icons&filters[project_categories][query][$contains]=next-js

const fetcher = (url: string) => fetch(url).then(res => res.json())

const Projects =  () => {
  const [queryLanguage, setQueryLanguage] = useState<any>([])
  const { data, error, isLoading } = useSWR(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/projects?populate[image]=*&populate[project_categories][populate]=icons${queryLanguage.map(query => `&filters[project_categories][query][$in]=${query}`)}`, fetcher)
  console.log(data)
  ...
  function filteredLanguageHandler(currentLanguage: any){
    if(!queryLanguage.some(lang => lang === currentLanguage.name)){
      setQueryLanguage([...queryLanguage, currentLanguage.name])
    }
    
    if(queryLanguage.some(lang => lang === currentLanguage.name)){
      const filteredQuery = queryLanguage.filter(query => query != currentLanguage.name)
                  ...
                  <div className="space-y-4 flex-1">
                    <div className="w-full h-[180px] relative rounded overflow-hidden">
                      ...
                      <div className="flex justify-end gap-2 flex-wrap absolute top-0 right-0 p-2">
                        {project.attributes.project_categories.data.map(icon => {
                          return (
                            <div className="p-1 flex gap-2 bg-accent">
                              <DynamicSvgIcon name={icon.attributes.icons.data[0].attributes.name} className="w-4 fill-black" />
                            </div>
                          )
                        })}
                      </div>
